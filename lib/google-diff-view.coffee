...
class GitDiffView
  constructor: (@editor) ->
    ...
    @subscriptions.add atom.commands.add editorView, 'google-diff:move-to-next-diff', =>
      @moveToNextDiff()
  ...
  addDecorations: (diffs) ->
    for {oldStart, newStart, oldLines, newLines} in diffs
      ...
      if oldLines is 0 and newLines > 0
        @markRange(startRow, endRow, 'google-line-added')
      else if newLines is 0 and oldLines > 0
        @markRange(startRow, startRow, 'google-line-removed')
      else
        @markRange(startRow, endRow, 'google-line-modified')
    return
